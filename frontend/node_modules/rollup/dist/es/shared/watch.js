...
*/
import { getAugmentedNamespace, fseventsImporter, getDefaultExportFromCjs, waitForAllAndRethrowFirstFailure, createFilter, rollupInternal } from './node-entry.js';
...
class Watcher {
    constructor(optionsList, emitter) {
        ...
        process$1.nextTick(() => this.runExclusively(() => this.run()));
    }
    async close() {
        if (this.closed)
            return;
    ...
    async announceChangeBatch() {
        ...
        await waitForAllAndRethrowFirstFailure(changes.flatMap(([id, event]) => [
